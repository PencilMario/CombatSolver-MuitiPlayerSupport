...
- 遗物优先级只作用于既有 HP/额度轴后的达标偏好，默认 1；3 高于所有低档内置目标之和。新增半血目标时，阈值内/外的目标掩码必须参与原 Pareto，不能让高生命未达标状态支配低生命已达标状态。MeatOnTheBone 仅在用户开启目标且完整获胜时补入实际回血与原单调回血的差额，并沿首领 HP 价值折算，禁止再向实际回血或生命值重复加一次。
