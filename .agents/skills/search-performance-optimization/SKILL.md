...
- 成长策略由主线程冻结每次成功收益的 HP 额度。完整胜利后比较扣除额度的战略战损，同分优先已实现成长再比较回合；零额度也优先免费收益。有实际成长目标时不可使用纯 HP 的早停或 incumbent 下界；只有本场持有对应卡牌才算有目标，保存非零额度本身不算。默认开启的战损目标早停在合法完整胜利候选提交时触发，排空当前父节点/并行批次后收尾，补充搜索同样检查；增量回放须核对收益次数和额度，Pareto 不得以较少收益支配较多收益。
