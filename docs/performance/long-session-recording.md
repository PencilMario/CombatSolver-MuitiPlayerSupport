...
3. 地图、出牌或第二局开局出现持续卡顿时，按一次 **Ctrl＋F8**。这会留下时间标记并保存当时的进程内存，可能短暂停顿；采集器记录快照起止，分析时排除这段人为停顿。该动作完全在本地执行，不上传。
4. 最后正常退出整个游戏，等采集器收尾。保留本次进程目录中的全部文件；只给普通 `godot.log` 会缺少调用栈和内存证据。红色提示表示记录失败；黄色常见于启动或分段切换，持续黄色表示录制中断。

内存快照可包含该进程当时的完整数据，用于本地对象引用分析。全程轨迹可能达到数十 GB，采集器每两秒检查磁盘，剩余不足 5 GB 时明确停止。不会删除之前的段，也不会只保留最后一场战斗。移除启用文件并重启游戏即可关闭诊断。
...
| `trace-*.nettrace` | 连续分段的托管线程采样、GC、分配抽样、异常、锁竞争、线程池与符号信息 |
...
`tools/export-performance.ps1 -SessionDirectory <目录>` 在采集完成后创建相邻 ZIP。原始目录保留。异常退出、工具报错时保留原始目录供分析，不伪造正常完成标记。
