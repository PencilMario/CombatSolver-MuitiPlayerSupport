...
| 多人模式当前回合 | 开发中 | 自动识别 Host/Client；Overlay 在战斗开始及原版操作锁未解除时显示等待状态，并在首回合最多等待三秒直至本地状态稳定。多人本地回合以原版 `PlayerActionsDisabled=false` 判断并触发搜索、手动计算和部署，不依赖可能滞后的 `PlayerCombatState.Phase` 或全局 `CurrentSide`；单人仍按全局 `CurrentSide=Player` 与 `Phase=Play` 校验；不保留跨回合 continuation，不介入其他玩家回合；多人专属选择、队友死亡后的 Hook 活性和多人卡牌仍不支持。控制器边界断言已覆盖，尚待多人实机验证。 |
