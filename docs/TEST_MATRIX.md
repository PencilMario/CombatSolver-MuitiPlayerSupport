...
- 启动采样修复：`startup.test.mjs` 覆盖重启不覆盖原分钟值、前 60 秒与 TTL 前一毫秒不写入、满 TTL 恢复采样、稳定无人时真实 0 人；通过。既有聚合测试改为恢复完成后的整分钟开始，保留聚合/过期/重启断言并通过。浏览器 `startup recovery` 合同通过，恢复期显示状态而非虚假 0。
