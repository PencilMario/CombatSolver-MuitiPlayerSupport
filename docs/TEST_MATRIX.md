...
- 上述最终夹具在原生 `Hook.AfterEnergyReset` 上对照完整状态，同场覆盖 Genesis/StarNextTurn/Radiance 的星能、能量和层数变化；验证不同获得顺序具有不同分支指纹及续用文本，Fork 保持顺序。未运行满球槽激发、移除后重新获得能力、原包整场回放；该证据不代表这些边界通过。
