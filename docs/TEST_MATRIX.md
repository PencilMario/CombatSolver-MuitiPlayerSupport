...
- 发布范围冻结在已验证行为提交 `d8ae412`：前两批18类机制及2张牌估值。后续木乃伊之手仅有诊断场景，没有验证成立的修复，已从发布源码移出。0.37.1仅同步版本与中英玩家日志，沿用下列已完成的行为证据；官方名称从当前游戏PCK读取。准备阶段执行一次Release构建和最小ZIP，不重复行为测试或运行完整发布门禁。
