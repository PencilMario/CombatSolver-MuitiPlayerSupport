...
- 顶部拆行与动作列表排版：`UI-PRIORITY-FEEDBACK` / `885119abd0f14b39ac773ff7c2c3fbf8`，Passed，22.58 秒；真实 Godot 控件中第 9、12 回合状态 GetLineCount=1 且 ClipText=false，既有收起/设置合同通过。`UI-LOCALIZATION` / `f48d695e14ac407cbe01e9c2d55269c5`，Passed，9.70 秒，同进程复用，覆盖 eng/zhs/zht、胶囊语言往返、tooltip、订阅清理。字体已启用 tnum 特性，但没有对所有字体做数字像素宽度测量。未收到截图，未验证层叠冲突或真实视觉观感；横幅与透明度保持原配置。测试后仅修正 UI 初始化日志的布局标记，再生成最终 Release。
