...
| `ReachableHandPotential.cs` | 手牌估值的双资源 0/1 背包；仅消费本次查询得到的费用和值，不保存分支引用或跨请求缓存 |
...
`BeamRetentionPolicy` 的 `RoutingChoiceScratch` 只复用一张路由签名字典的空桶。每次 `RankBest` 新建 `RoutingChoiceNodes`，把候选有序列表和原五项代表放在同一组内；首次节点初始化代表，后续仍调用原比较规则。组不池化，归还 scratch 时清空节点引用；分组之外的族/选项顺序与配额照旧，父链/评分查询保留原调用方式。这些临时聚合不进入战斗状态键或续用戳。
...
普通搜索按进程可用逻辑处理器数量选择初始展开 lane：至少 16 个时默认 DOP8，4–15 个时默认 DOP4，2–3 个时默认 DOP2，只有 1 个时使用 DOP1；用户显式设置始终优先。设置中的“关闭（单线程）”映射 DOP1，数值项为 `2..16`，实际值还会按进程可用逻辑处理器钳制。多父节点由最多 DOP 个低优先级后台 lane 执行，coordinator 独占按序提交；自然单父节点仍由 coordinator 执行并借用 DOP−1 个空闲 lane，两个模式不嵌套。lane 在一次 `Solve` 内复用 solver、缓存和 `SearchWorkPacer`。worker 不写全局 transposition、dominance 或 fallback：它们只物化原始候选，coordinator 仍按父节点输入顺序提交，因此固定节点预算下 DOP 不改变搜索语义。详细诊断和增量严格回放强制 DOP1。
...
并行搜索失败提示会保留本次请求的 DOP；DOP 大于 1 时先引导上传问题包，再建议切换为“关闭（单线程）”。并行阶段指标为各 lane 的 `Stopwatch` 经过时间之和，包含等待/抢占/GC，可以超过请求墙钟耗时，但不是实际 CPU 时间；实际用核需由线程调度运行时间或 on-CPU 采样另行测量。`parallel_waves / work_items / max_concurrency`、`parallel_action_*` 与 `parallel_round_choice_*` 分别证明父节点、自然 singleton action 和宽选择层并发实际发生，`deferred_round_choice_*` 记录命中层宽与有限配额回退。外层 wave 用完成队列接收各 lane 的 outcome。coordinator 先归并该 worker 的指标，再给空闲 lane 派发同一预约窗口中的下一父节点；独立的已接收位图只允许按输入序号提交连续前缀，并释放其 raw batch 和父模拟器。最多 DOP 个父节点同时模拟，已预约/等待提交的窗口最多 `2×DOP`，不是固定字节界；内存预约仍按全部已准入父节点计费。异常停止继续派发，补齐未派发计数并排空全部已派发工作后释放批次；额外父节点 lane 也参加缓存检查点与 Dispose。单个 parent 内 action/round-choice aggregate 仍可能持有完整原始候选；高于默认值属于用户主动的速度、CPU 与峰值内存权衡。节点预算截断时，coordinator 立即释放未展开父节点和不会进入下一层的候选模拟器，并用 `node_limit_snapshots_released` 记录实际释放数。
