...
`SmartLayerMemoryForecast` 只决定有证据能改善容量的可选层间回收：完整层超过新区域容量、缺少预测或当前区域新分配低于 max(64 MiB, 区域限额/4)时沿用每批准入。`SearchGcPolicy` 根据最近完成的堆统计选择一次收集：碎片至少 1 GiB 且至少占堆一半时压缩，否则沿用已确认完成的后台收集；搜索中和搜索后共用选择器，手动回收继续强制压缩。算法层不调用 GC。
