...
/// </para>
/// <para>
/// 后果要分清。<b>对续接无害</b>：续接戳的 Power 段
/// （<c>ContinuationStamp.AppendPowers</c>）实机侧和模拟侧共用同一个方法，只读
/// <c>DynamicVars</c>，两边看到的东西一样，所以隐藏状态压根不进戳、也就不会对不上。
...
/// </para>
/// <para>
/// <b>隐藏状态不要往续接戳里塞。</b> 那个方法两侧共用，而
/// <c>PowerModel.DeepCloneFields</c> 把 <c>_internalData</c> 重置成
/// <c>InitInternalData()</c>——模拟克隆读到的是初值，实机实例读到的是真值，塞进去只会让每一回合
/// 的续接都对不上。真要进戳得像遗物那样拆成实机版和预测版两个追加方法，本入口不做这件事。
/// </para>
/// <para>
