...
{
    private async Task AssertSearchObjectiveTargetStopAsync(CombatState combat)
    {
        ...
        {
            Objective = new(SearchObjective.PermanentGrowth, 0, 30, 2),
            IgnoreLongTermRewards = false,
            PotionPolicy = SolverPotionPolicy.Disabled,
            ForceShortOnly = true,
            ShortBudgetOverrideMilliseconds = 2000,
            MaxDegreeOfParallelism = 1,
            VerifyIncrementalSearch = true,
            Diagnostics = new(message => messages.Enqueue(message), _ => { }),
        };
        SolverResult result = await Task.Run(() => CombatSearchCoordinator.Solve(
            root, names, damage, policy, CancellationToken.None, null));
        if (!result.CombatEndedTurn.HasValue || !result.Snapshot.AllEnemiesDead
            || result.Snapshot.PlayerDead || result.Snapshot.Objective.Growth.CardBlock < 2
            || !result.Snapshot.Objective.CanStopSearch(completeVictory: true)
            || !CombatSearchCoordinator.HasReachedAcceptableBattleHpLoss(policy, result)
            || !messages.Any(message => message.Contains("OBJECTIVE_TARGET_REACHED", StringComparison.Ordinal)))
            throw new InvalidOperationException($"Growth target stop mismatch: turn={result.CombatEndedTurn} dead={result.Snapshot.PlayerDead} enemiesDead={result.Snapshot.AllEnemiesDead} outcome={result.Snapshot.Objective}; messages={string.Join(" | ", messages.Where(message => message.Contains("OBJECTIVE", StringComparison.Ordinal)))}");
        _completedChecks.Add("SearchObjectives:ActualTargetReached:SolverStop:CoordinatorStop:IncrementalReplay");
    }
