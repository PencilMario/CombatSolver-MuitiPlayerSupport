...
test('live sorting is global before pagination and preserves online-time rank',async()=>{
  ...
  try{
    for(let n=1;n<=35;n++)assert.equal((await post(c+'/v1/heartbeat',{sessionId:n.toString(16).padStart(32,'0'),name:'player '+n,character:'SILENT',floor:n,encounter:'A',hpLoss:36-n,version:'test'})).status,204);
    assert.equal((await post(c+'/v1/heartbeat',{sessionId:'f'.repeat(32),name:'unknown',character:'',floor:null,encounter:'',hpLoss:null,version:'old'})).status,204);
