...
for (int trial = 0; trial < 10_000; trial++)
{
    ...
    Require(ReachableHandPotential.Calculate(cards, energy, stars) == expected,
...
for (int iteration = 0; iteration < 1000; iteration++)
    _ = ReachableHandPotential.Calculate(smallHand, 2, 2);
...
static void Check((int Energy, int Stars, int Value)[] cards, int energy, int stars)
{
    ...
    Require(ReachableHandPotential.Calculate(cards, energy, stars) == old[energyCapacity, starCapacity],
