...
> **当前工作重点（2026-09-13）：** 本分支`perf/hotspot-exploration-20260913`的已完成性能优化由PR #90正式评审，已整合上游0.38.0；全部尝试与收益边界见`docs/performance/pr90-attempts-20260913.md`。可恢复选牌执行器另在PR #91、`perf/resumable-choice-executor-20260913`独立草稿推进，不继续混入本分支。不移植Compact，不改变搜索预算或决策质量；本批不提升版本、不发包或上传。
