...
当前源码版本为 **0.33.1**：增加可关闭的在线状态统计与作者管理后台。详见 [更新日志](docs/releases/0.33.1-RELEASE_NOTES.md)；创意工坊发布状态以实际发布记录为准。
...
- **在线统计**：默认每 30 秒向作者发送随机安装标识、昵称、角色、楼层、当前战斗、预计战损和版本，可在设置中关闭；不上传完整路线，离线后清除玩家详情，历史仅保留人数。详见 [统计字段与关闭方式](docs/ONLINE_STATISTICS.md)。
